...
class BureauAggregationV2(FeatherFeatureIDFromDF):
    ...
    def create_features_from_dataframe(bur: pd.DataFrame, random_state=None):
        ...
        bur['BUR_CREDIT_ACTIVE_BINARY'] = \
            (bur.CREDIT_ACTIVE == 'Active').astype(int)
        bur['BUR_CREDIT_ACTIVE*DAYS_CREDIT'] = \
            (bur.BUR_CREDIT_ACTIVE_BINARY * bur.DAYS_CREDIT)
        bur['BUR_CREDIT_ACTIVE*DAYS_CREDIT_ENDDATE'] = \
            (bur.BUR_CREDIT_ACTIVE_BINARY * bur.DAYS_CREDIT_ENDDATE)
        bur['log(AMT_CREDIT_SUM_DEBT)/log(DAYS_CREDIT_ENDDATE)'] = \
            np.log(bur.AMT_CREDIT_SUM_DEBT + 0.001) / np.log(bur.DAYS_CREDIT_ENDDATE + 0.001)
        ...
        for agg in ['mean', 'min', 'max', 'sum', 'var', 'skew']:
            for select in ['log(AMT_ANNUITY)',
                           ...
                           'DAYS_CREDIT',
                           'BUR_CREDIT_ACTIVE*DAYS_CREDIT',
                           'DAYS_CREDIT_ENDDATE',
                           'BUR_CREDIT_ACTIVE*DAYS_CREDIT_ENDDATE',
                           'DAYS_ENDDATE_FACT',
                           'DAYS_CREDIT_UPDATE']:
